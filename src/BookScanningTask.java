...
//		}
		...
		for (String fileName : inputFileNames) {
			...
			printAns(scoresCollected.get(0).getAnswerString(),  DIRECTORY_PATH+"BEST_REACHED_SOL_"+fileName);
		}
	}

	public synchronized static Score getSumScoreBySignUpSort(String methodologyName,ArrayList<Library> orgLibsList ,int time) {
	...
	public synchronized static Score getSumScoreByShipPerDaySort(String methodologyName,ArrayList<Library> orgLibsList,int time) {

		ArrayList<Library> ourOwnLibs = (ArrayList<Library>) orgLibsList.clone();
		Collections.sort(ourOwnLibs, comparatorCompareByshipPerDay);
		//System.out.println("Get score after sorting by #books could be shipped per day starting..");
		Score score = getSumScoreOf(ourOwnLibs,time);
		score.setMethodologyName(methodologyName);
		return score;
	...
	public synchronized static Score getSumScoreUniqueBooksOrder(String methodologyName,	ArrayList<Book> orgBooksList,int time) {
		...
		//System.out.println("Get score after sorting all books by number of libs contain.");
